"""
Train an model using study level dataloader
"""
import numpy as np
...
class Trainer:
    ...
    def batch_forward(self, X, Y, Ymask, lengths, meta):
        ...
        for length,label in zip(lengths,Y):
            studfeatures = X[offset:offset+length].mean(0).unsqueeze(0)
            studpred = self.model(studfeatures)
            prediction.append(studpred)
            offset += length
