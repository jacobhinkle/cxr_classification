...
class MIMICCXRJPGDataset(Dataset):
    def __init__(
        ...
        image_subdir_256="features256x256",
        image_subdir_512="features512x512",
        image_subdir_1024="features1024x1024",
        image_subdir_2048="features2048x2048",
        label_method="zeros_uncertain_nomask",  # old default: "ignore_uncertain"
        load_activations=True,  # If True, load .pth files and do not apply transforms
    ):
        super().__init__()
        ...
        # Let's have four different directory for four resolution 
        self.data256 = Path(datadir) / image_subdir_256
        self.data512 = Path(datadir) / image_subdir_512
        self.data1024 = Path(datadir) / image_subdir_1024
        self.data2048 = Path(datadir) / image_subdir_2048
    ...
    def get_from_row(self, row):
        if self.load_activations:
            ...
            pthpath = b + ".pt"
            im256 = torch.load(self.data256 / pthpath)
            im256 = im256.unsqueeze(0)
            im512 = torch.load(self.data512 / pthpath)
            pool512 = torch.nn.AvgPool2d((2,2))
            im512 = pool512(im512).unsqueeze(0)
            im1024 = torch.load(self.data1024 / pthpath)
            pool1024 = torch.nn.AvgPool2d((4,4))
            im1024 = pool1024(im1024).unsqueeze(0)
            im2048 = torch.load(self.data2048 / pthpath)
            pool2048 = torch.nn.AvgPool2d((8,8))
            im2048 = pool2048(im2048).unsqueeze(0)
            
            im = torch.cat((im256,im512,im1024,im2048),0)
